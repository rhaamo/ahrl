...
class EditQsoForm(BaseQsoForm):
    ...
    qsl_rcvd = SelectField(
        "QSL Received",
        choices=[["N", "No"], ["Y", "Yes"], ["R", "Requested"], ["I", "Invalid (Ignore)"], ["V", "Verified (Match)"]],
    )
    ...
    lotw_qsl_sent = SelectField(
        "LOTW QSL Sent",
        choices=[["N", "No"], ["Y", "Yes"], ["R", "Requested"], ["Q", "Queued"], ["I", "Invalid (Ignore)"]],
    )
...
class AdvSearchForm(Form):
    ...
    pictures = SelectField(
        label="Has pictures", validators=[Optional()], choices=[["any", "Any"], ["Y", "Yes"], ["N", "No"]]
    )
    # qsl statues <select>
    # eqsl statues <select>
    # pictures <select>
